...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        private float ProjectileColliderSpeed = 25.0f;
        ...
        {
            setSpeed(PersistentData.data.myWand);
            base.Start();

            StartCoroutine(SendCollisionAfterDelay());
        }

        public void setSpeed(string wandType)
        {
            if (wandType.Equals("PearlWand"))
            {
                ProjectileColliderSpeed = 25.0f;
            }

            else if (wandType.Equals("FlameWand"))
            {
                ProjectileColliderSpeed = 35.0f;
            }

            else if (wandType.Equals("AshWand"))
            {
                ProjectileColliderSpeed = 15.0f;
            }
